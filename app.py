...
@app.route("/delete-tool", methods=["POST", "GET"])
def deleteTool():
...
@app.route('/edit-tool', methods=['POST', 'GET'])
def editTool():
    print(request)
    print(request.args)
    tool = Tool.query.get(request.args)
    if request.method=="POST":
        ...
        tool.content = request.form['content']
        tool.link = request.form['link']
        db.session.commit()
        return redirect("/")
    return render_template('edit-tool.html', tool=tool)   
